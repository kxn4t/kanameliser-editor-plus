...
5. **類似名マッチ** — 構造的サフィックス除去後の名前一致（例: `Body_01` ≈ `Body_02`）または共通ベース名の一致（例: `Ribbon_blue` ≈ `Ribbon_red`）を複合スコアでランキング
   - 例: `Ribbon_blue` → `Ribbon_red`、`Hair.001` → `Hair.002`

同じ優先度で複数の候補が残る場合は、階層パスの類似度（末端側から親オブジェクトを比較）、祖先コンテキストの一致度、深度の近さ、レーベンシュタイン距離の順に判定されます。

この仕様はMA Material Helperのマッチングでも共通です。

#### 詳細マッチングログ

自動マッチングが期待通りの結果にならない場合、Unityコンソールにマッチング判定の詳細情報を出力できます。このトグルはMA Material Helperのマッチングにも適用されます。
...
- 鞄などON/OFF切り替えのあるメッシュは、一緒にON/OFFされる単位で統合してください（MA Object Toggleの切り替え単位など）。常時表示のメッシュや別トグルのメッシュと混ぜるとトグルが動作しなくなります（該当する場合はビルド時にAAOが警告を表示します）。メッシュを個別にON/OFFしている場合は、統合後にMerge Skinned Meshの「有効無効状態に関するアニメーションをコピーする」を有効にしてください
