...
- **Component Copier** — Mirrored Transform copies now account for negative parent scales and rotated child axes under non-uniform scales, preserving the mirrored rotation and world size when updating or creating a counterpart. The rest rotation of constraints is mirrored the same way.
...
- **Component Copier** — Transform の左右反転で、親の負のスケールと非均等スケール下の子の回転を考慮するようにしました。対応先の更新時・新規作成時ともに、反転した回転とワールド空間の大きさを保ちます。Constraint の静止時の回転（Rotation At Rest）も同じように反転します。
...
- **Component Manager**
  ...
  - The window now follows hierarchy changes and undo/redo, so the list no longer goes stale when components are added or removed outside the window (stale rows could previously be checked but were silently skipped on deletion)
  - When a component and another component that requires it (`RequireComponent`) are deleted together, the dependent is now removed first so both are deleted regardless of order; components that still cannot be deleted are reported instead of being silently left behind
- **MA Material Helper**
