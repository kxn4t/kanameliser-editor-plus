---
layout: home

hero:
  name: Kanameliser Editor Plus
  text: Editor Extensions for Unity & VRChat
  tagline: A collection of tools to streamline avatar modification
  actions:
    - theme: brand
      text: Get Started
      link: /en/guide/getting-started
    - theme: alt
      text: BOOTH
      link: https://kanameliser.booth.pm/items/6751267
    - theme: alt
      text: GitHub
      link: https://github.com/kxn4t/kanameliser-editor-plus

features:
  - title: Mesh Info Display
    details: Shows polygon count, material count, and mesh count for selected objects in the Scene view. Supports NDMF preview.
    link: /en/features/mesh-info-display
  - title: Toggle Objects Active
    details: Quickly toggle between GameObject active state and EditorOnly tag with Ctrl+G.
    link: /en/features/toggle-objects-active
  - title: Material Copier
    details: Copy & paste materials between GameObjects with matching names.
    link: /en/features/material-copier
  - title: FBX Settings Copier
    details: Copy & paste FBX import settings between FBX assets, with batch paste support.
    link: /en/features/fbx-settings-copier
  - title: MA Material Helper
    details: Auto-generate Modular Avatar color-change menus from color variation prefabs.
    link: /en/features/ma-material-helper
  - title: Merge Skinned Mesh (Color Menu Safe)
    details: Auto-configure an AAO Merge Skinned Mesh that keeps color change menus working.
    link: /en/features/color-menu-safe-merge
  - title: Component Manager
    details: Lists all components on selected objects and their children. Filter, multi-select, and batch delete.
    link: /en/features/component-manager
  - title: AO Bounds Setter
    details: Batch configure Anchor Override, Root Bone, and Bounds across multiple meshes.
    link: /en/features/ao-bounds-setter
---
