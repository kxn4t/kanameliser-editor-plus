...
鞄やアクセサリーなど、トグル（MA Object Toggleなど）でON/OFFするメッシュは、**基本的に統合対象へ含めず**、常時表示のメッシュだけを統合してください。統合したメッシュは1つのレンダラーになるため、常時表示のメッシュと混ぜて統合するとトグルが正しく動作しなくなります（該当する場合はビルド時にAAOが警告を表示します）。
