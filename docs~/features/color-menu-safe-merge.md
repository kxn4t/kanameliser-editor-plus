...
統合したメッシュは1つのレンダラーになるため、鞄やアクセサリーなどON/OFF切り替えのあるメッシュは、**一緒にON/OFFされる単位で統合してください**。MA Object Toggleなどでトグルを作っている場合も同様です。常時表示のメッシュや別のトグルで切り替えるメッシュと混ぜて統合すると、トグルが正しく動作しなくなります（該当する場合はビルド時にAAOが警告を表示します）。

単位ごとに統合した上で、トグルの作り方に応じて次のようになります:

- **トグルが共通の親オブジェクトをON/OFFしている場合**: 統合オブジェクトはその親の下に作成されるため、そのまま動作します
- **トグルがメッシュを個別にON/OFFしている場合**: 統合オブジェクトはトグルの対象にならないため、作成されたMerge Skinned Meshの「有効無効状態に関するアニメーションをコピーする」を有効にしてください（すべての統合元が同じアニメーションでON/OFFされている場合に使用できます）
